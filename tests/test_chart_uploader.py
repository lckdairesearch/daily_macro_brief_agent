...
from app.render.chart_uploader import _object_key, publish_chart, upload_chart_to_r2
...
def test_object_key_preserves_repo_relative_path(tmp_path):
    ...
    try:
        assert _object_key(chart) == "charts/outputs/runs/2026-05-09/test-run/chart.png"
    finally:
...
def test_upload_chart_to_r2_puts_object_and_returns_public_url(tmp_path):
    ...
    assert url.startswith("https://pub.example.com/charts/chart.png?ts=")
    assert client_calls["Bucket"] == "brief-charts"
    assert client_calls["Key"] == "charts/chart.png"
